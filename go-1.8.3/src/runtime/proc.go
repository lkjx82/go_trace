...
func newproc(siz int32, fn *funcval) {
	...
	// gid := Getgid() // getg().goid
	oldg := getg()
	systemstack(func() {
		newproc1(fn, (*uint8)(argp), siz, 0, pc, oldg)
	})
}
...
func newproc1(fn *funcval, argp *uint8, narg int32, nret int32, callerpc uintptr, parentg *g) *g {
	_g_ := getg()
	...
	// newg.forTrace = genForTrace(newg)
	// lbh trace 调用协程执行的hook
	onGStartHook(newg, parentg)
	// onGStart(newg, parentg)
	// onGStartHook(newg, _g_.goid)
